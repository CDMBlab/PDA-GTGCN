...
def constructNet(RNA_dis_matrix):
    RNA_matrix = np.matrix(
        np.zeros((RNA_dis_matrix.shape[0], RNA_dis_matrix.shape[0]), dtype=np.int8))
    dis_matrix = np.matrix(
        np.zeros((RNA_dis_matrix.shape[1], RNA_dis_matrix.shape[1]), dtype=np.int8))
    mat1 = np.hstack((RNA_matrix, RNA_dis_matrix))
    mat2 = np.hstack((RNA_dis_matrix.T, dis_matrix))
    adj = np.vstack((mat1, mat2))
    return adj

def constructHNet(RNA_dis_matrix, RNA_matrix, dis_matrix):
    mat1 = np.hstack((RNA_matrix, RNA_dis_matrix))
    mat2 = np.hstack((RNA_dis_matrix.T, dis_matrix))
